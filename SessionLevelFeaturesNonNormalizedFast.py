...
import h5py
data = h5py.File ('/Users/ofekh/Library/CloudStorage/OneDrive-Bar-IlanUniversity-Students/MachineLearning_HW/FinalProject_HelperFunctions/region_sessions_Notnormalized.mat')
dataAA = h5py.File('/Users/ofekh/Library/CloudStorage/OneDrive-Bar-IlanUniversity-Students/MachineLearning_HW/FinalProject_HelperFunctions/region_sessions_AA_Notnormalized.mat')
dataNatMixes = h5py.File('/Users/ofekh/Library/CloudStorage/OneDrive-Bar-IlanUniversity-Students/MachineLearning_HW/FinalProject_HelperFunctions/region_sessions_Nat_Notnormalized.mat')
list(data.keys())
...
def build_normalized_dataset(condition, X_pcx_sessions, X_plcoa_sessions, subsample=False, random_state=42):
    """Build normalized dataset, optionally subsampling to match total neuron count between regions."""
    structure = session_structure[condition]
    rng = np.random.default_rng(random_state)

    # Normalize all sessions first and keep track of labels
    def normalize_sessions(sessions, label):
        normalized_list = []
        for session in sessions:
            print(f"Processing {label} session shape:", session.shape)
            norm = normalize_session_per_odor_per_neuron(session, structure)
            normalized_list.append(norm)
        return normalized_list

    pcx_norm = normalize_sessions(X_pcx_sessions, 'PCx')
    plcoa_norm = normalize_sessions(X_plcoa_sessions, 'plCoA')

    if not subsample:
        # No subsampling; return full stack
        X_all = np.vstack(pcx_norm + plcoa_norm)
        y_all = np.array(['PCx'] * sum(x.shape[0] for x in pcx_norm) + ['plCoA'] * sum(x.shape[0] for x in plcoa_norm))
        return X_all, y_all

    # Step 1: Concatenate all neurons from each region
    pcx_full = np.vstack(pcx_norm)
    plcoa_full = np.vstack(plcoa_norm)

    # Step 2: Get min total neuron count
    min_total = min(len(pcx_full), len(plcoa_full))
    print(f"Subsampling both regions to {min_total} neurons total.")

    # Step 3: Subsample to equal total neuron count
    pcx_indices = rng.choice(len(pcx_full), min_total, replace=False)
    plcoa_indices = rng.choice(len(plcoa_full), min_total, replace=False)

    X_all = np.vstack([pcx_full[pcx_indices], plcoa_full[plcoa_indices]])
    y_all = np.array(['PCx'] * min_total + ['plCoA'] * min_total)
...
half1 = X_plcoa_Mono[:5]
half2 = X_plcoa_Mono[5:10]
...
X, y = build_normalized_dataset('Mono', X_pcx_AA, X_plcoa_AA,True)  #('Mono', X_pcx_Mono, X_plcoa_Mono)
...
print(f"\nReal accuracy: {real_acc:.4f}, p-value: {p_value:.7f}")
